...
class HealthDataStoreClass {
  ...
  async loadAllData(): Promise<void> {
        ...
        if (file.extension === '.txt') {
          ...
          if (!needsExtraction(manifest, relativePath, fileHash)) {
            ...
            if (cached && cached.sourceHash === fileHash) {
              ...
              console.log('[1health.ai] Loaded biomarkers from cache');
            ...
            if (pdfText) {
              ...
              if (biomarkerCount >= 10) {
                ...
                console.log(
                  `[1health.ai] Biomarker extraction successful (${biomarkerCount}), cached`
                );
              } else {
                console.warn(
                  `[1health.ai] Biomarker extraction returned only ${biomarkerCount} items, NOT caching`
                );
              }
            }
          }
        ...
        if (file.extension === '.txt') {
          ...
          if (!needsExtraction(manifest, relativePath, fileHash)) {
            ...
            if (cached && cached.sourceHash === fileHash) {
              ...
              console.log('[1health.ai] Loaded body composition from cache');
            ...
            if (pdfText) {
              ...
              if (hasData) {
                ...
                manifestChanged = true;
                console.log('[1health.ai] Body comp extraction successful, cached');
              } else {
                console.warn('[1health.ai] Body comp extraction returned empty, NOT caching');
              }
            }
          }
        }
    ...
    this.loaded = true;
    console.log('[1health.ai] Health data loaded successfully');
  }
