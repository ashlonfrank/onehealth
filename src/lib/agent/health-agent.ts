...
const HEALTH_SYSTEM_PROMPT = `You are 1health.ai, a knowledgeable health assistant that helps users understand their health data and make informed decisions about their wellness.
  ...
  try {
    ...
    for await (const msg of response) {
      switch (msg.type) {
        ...
        case 'system':
          if (msg.subtype === 'init') {
            console.log(`[1health.ai] Session started: ${msg.session_id}`);
          }
          break;
      }
    }
