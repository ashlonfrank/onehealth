...
export async function extractBodyCompWithAI(
  ...
  if (!apiKey) {
    console.warn('[1health.ai] No API key, skipping AI body comp extraction');
    return {};
  }
  ...
  try {
    ...
    console.log('[1health.ai] Extracting body composition with AI...');
    ...
    if (toolUse && toolUse.type === 'tool_use') {
      ...
      console.log(`[1health.ai] AI extracted ${fieldCount} body composition fields`);

      // Log key metrics for debugging
      if (extracted.bodyFatPercent) {
        console.log(`[1health.ai] Body Fat: ${extracted.bodyFatPercent}%`);
      }
      if (extracted.leanMass) {
        console.log(`[1health.ai] Lean Mass: ${extracted.leanMass} lbs`);
      }
      if (extracted.vatMass) {
        console.log(`[1health.ai] VAT: ${extracted.vatMass} lbs`);
      }

      return extracted;
    }

    console.warn('[1health.ai] No tool use in body comp response');
    return {};
  } catch (error) {
    console.error('[1health.ai] AI body comp extraction error:', error);
    return {};
  }
}
