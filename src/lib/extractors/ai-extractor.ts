...
export async function extractBiomarkersWithAI(text: string): Promise<ExtractedBiomarkers> {
  ...
  if (!apiKey) {
    console.warn('[1health.ai] No API key, skipping AI extraction');
    return {};
  }
  ...
  try {
    ...
    console.log('[1health.ai] Extracting ALL biomarkers with AI...');
    ...
    if (toolUse && toolUse.type === 'tool_use') {
      ...
      console.log(`[1health.ai] AI extracted ${biomarkers.length} biomarkers`);
      ...
      console.log(`[1health.ai] Mapped ${knownKeys.length} to known biomarker keys`);

      return extracted;
    }

    console.warn('[1health.ai] No tool use in response');
    return {};
  } catch (error) {
    console.error('[1health.ai] AI extraction error:', error);
    return {};
  }
}
